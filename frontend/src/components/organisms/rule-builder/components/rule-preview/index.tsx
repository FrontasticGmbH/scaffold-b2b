...
const RulePreview = ({ group, label, criteria }: RulePreviewProps) => {
  const constructPreview = useCallback((rule: Rule | Group, criteria: Criteria[]) => {
    if (rule.type === 'rule') {
      const ruleCriteria = criteria.find((c) => c.key === rule.key);

      const name = ruleCriteria?.name ?? rule.key;
