...
export class EmailApi {
  ...
  async sendAssociateVerificationAndPasswordResetEmail(customer: Account, passwordResetToken: AccountToken) {
    ...
    const htmlVerificationMessage = `
      <h1>Hello!</h1>
      <p style="margin-top: 10px;color:gray;">Please activate your account by clicking the link below</p>
      <a href="${verificationUrl}">${verificationUrl}</a>
    `;
  ...
  async sendWelcomeAssociateEmail(account: Account, businessUnit: BusinessUnit) {
    const htmlWelcomeCustomerMessage = `
      <h1>Hello!</h1>
      <p>We are so happy to have you here in ${businessUnit.name}!</p>
    `;
    await this.sendEmail({
      to: account.email,
      subject: 'Welcome',
